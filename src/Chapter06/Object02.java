...
class A {	//패키지 이름은 같은 패키지 내에서 동일한 이름을 쓸 수 없다. 오류가 난다.
	...
	//메소드 : 함수 , 호출시 작동됨.
	// 리턴타입(void) 메소드명(인풋매개변수) { 메소드 호출시 프로그램 실행 코드}
...
public class Object02 {
	...
	public static void main(String[] args) {
		...
		//자료형(객체)
		//클래스명(A) 참조변수(a) = new A();
		// A : 클래스, 객체 자료형 , 동일한 패키지 내에 클래스가 존재할때 import 없이 사용 가능
		// a : 참조변수 (Stack(주소) : Heap ( 필드, 메소드 ) , 객체
		// new : Heap 공간에 필드와 메소드를 저장 하라.
		// A() : A 클래스의 생성자 호출
		
		
		// 1. 객체 생성 : A : class, a : Object (객체), A를 틀로 해서 만들어진 객체
		A a = new A();
		
		// 2. 필드 출력
		System.out.println("======= a 객체의 필드의 값을 출력 ========");
		System.out.println(a.m); //a.하면 필드와 메소드를 볼 수 있다.
		...
		a.print();  	// a 객체의 print() 메소드 호출 a.p
		
		System.out.println("======================");
		//하나의 클래스를 생성하면 클래스를 기준으로 여러개의 객체를 생성 할 수 있다.
		//클래스는 객체를 만들기 위해 생성한다.
