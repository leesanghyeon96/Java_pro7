...
public class String_Method {
	...
	public static void main(String[] args) {
		// String 클래스에서 사용되는 주요 메소드 (함수)
			...
			//참조자료형 : 배열 /열거 / 객체 / 인터페이스		<== 첫자가 모두 대문자
		
		
		
		// 1. 문자열의 길이 ( length() ) 
		//[참조]자료형   [참조]변수
		String str1 = "Hello Java!";		//방번호 : 0번=H, 1번=e
		String str2 = "안녕하세요! 반갑습니다.";
		
		System.out.println(str1.length());	//글자의 갯수(공백도 포함) : 11자
		System.out.println(str2.length());	// 13자
		
		
		
		//2. 문자열 검색 ( charAt(), indexOf(), lastIndexOf() )
		System.out.println(str1.charAt(1));		//charAt(방번호), 1번방의 문자를 출력 : e
		System.out.println(str2.charAt(1));		//녕
		
		System.out.println();
		
			//indexOf(), lastIndexOf() : <<한 문자의 방 번호를 검색>>
		System.out.println(str1.indexOf('a'));	//''가 들어감, 문자 a 가 저장된 방 번호를 출력 : 7
		System.out.println(str1.lastIndexOf('a'));	//9
		//indexOf()는 앞에서 부터 검색, lastIndexOf()는 뒤에서 부터 검색
		
		System.out.println(str1.indexOf('a', 8));	//a를 검색하는데 8번방부터 검색, 9 -->
	...
	//	String str4 = true;	//오류발행 : true (boolean 형 타입을 String 변수에 저장시 오류발생)
		...
		String str7 = "안녕" + 3;		//안녕3 - 3은 String 형으로 자동 변환
		...
		String str10 = "Hello Java!";		//방번호 : 0번=H, 1번=e
		...
		//toCharArray() : 문자열을 한글자씩 잘라서 arr3 배열 안에 넣는다.
		...
		//split() : 특정 문자열을 기준으로 잘라서 문자열 배열에 저장 ***중요
		String[] strArray = "abc/def-ghi jkl".split("/|-| ");
		//자르면 index0번방부터 차례대로 들어감 -> [abc, def, ghi, jkl]출력
		...
		System.out.println("         abc         ".trim());	// ->abc공백 사라짐
		...
		// == : stack 메모리의 참조 주소 비교. (new를 써서 주소가 전부 다르다)
